...
const jsonPane = document.getElementById('js-json-toggle').addEventListener('change', e => {
    ...
    if (!e.target.checked) { // Reset app layout to normal when unchecked
    ...
    try {
        ...
        if (seeJson) {
            ...
            for (let i = 0; i < docCount; i++) {
                tempString += JSON.stringify(json.docs[i], null, 2);
            }
            document.getElementsByClassName("json-content")[0].innerHTML = "<pre>" + tempString + "</pre>";
